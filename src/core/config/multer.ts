...
import { randStr } from '../helpers/file';
...
export const multerOptions = {
   ...
   fileFilter: (req: Request, file: Record<any, any>, cb: (result, isValid) => void): void => {
      if (
         file.mimetype.match(
            /(jpg|jpeg|png|gif|plain|msword|document|ms-excel|sheet|ms-powerpoint|presentation|csv|mp3|mp4|wma|acc|wav|flac|mpeg|audio)$/,
         )
      ) {
