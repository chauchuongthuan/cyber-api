...
export async function convertContentFileDto(
   dto: any,
   files: any,
   multiFileField?: Array<string>,
   validations?: Record<any, any>,
   ...
   console.log('🚀 ~ file: content.ts:35 ~ files:', files);
   const contentImgs = {};
   const otherFiles = {};
   let errors = [];
...
export async function trackingFileContent(
   content: any,
   doc: any,
   field: string,
...
export async function forEachAny(data: Record<any, any> | Array<any>, cb: (arg1: any, arg2: any) => void) {
...
export async function saveAudio(doc: any) {
   if (typeof doc.thumbnail === 'function') {
      const audio = doc.thumbnail();
      const field = 'audioFile';
      await fileHelper.saveAudio(doc, field, audio['collection']);
   }
}
export async function saveThumbOrPhotos(doc: any) {
   if (typeof doc.thumbnail === 'function') {
      ...
      if (thumbnail && thumbnail['fields'] && thumbnail['collection']) {
         await Promise.all(
            Object.keys(thumbnail['fields']).map(async function (field) {
               const compressOption =
                  typeof thumbnail['compress'] != 'undefined' && typeof thumbnail['compress'][field] != 'undefined'
                     ? thumbnail['compress'][field]
                     : {};
               await fileHelper.saveThumb(
                  doc,
                  field,
                  thumbnail['collection'],
                  thumbnail['fields'][field],
                  thumbnail['method'] || 'inside',
                  compressOption,
               );
            }),
         );
      ...
      if (thumbnail && thumbnail['fieldTrans'] && thumbnail['collection']) {
         await Promise.all(
            Object.keys(thumbnail['fieldTrans']).map(async function (field) {
               const compressOption =
                  typeof thumbnail['compress'] != 'undefined' && typeof thumbnail['compress'][field] != 'undefined'
                     ? thumbnail['compress'][field]
                     : {};
               await fileHelper.saveThumbTrans(
                  doc,
                  field,
                  thumbnail['collection'],
   ...
   if (typeof doc.thumbPhotos === 'function') {
      ...
      if (thumbPhotos && thumbPhotos['fields'] && thumbPhotos['collection']) {
         await Promise.all(
            Object.keys(thumbPhotos['fields']).map(async function (field) {
               const compressOption =
                  typeof thumbPhotos['compress'] != 'undefined' && typeof thumbPhotos['compress'][field] != 'undefined'
                     ? thumbPhotos['compress'][field]
                     : {};
               await fileHelper.savePhotos(
                  doc,
                  field,
                  thumbPhotos['collection'],
                  thumbPhotos['fields'][field],
                  thumbPhotos['method'] || 'inside',
                  compressOption,
               );
            }),
         );
...
export function validationMultiFile(
   file,
   validations: Record<any, any>,
   messages: Record<any, any>,
